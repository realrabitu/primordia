class_name CameraBoundsServiceGd
extends RefCounted


static func update_camera_limits(level_node: Node2D, generated_root: Node2D, generated_bounds: Rect2, players: Array, margin_x: float, margin_top: float, margin_bottom: float, tile_layer: TileMapLayer = null, tile_map: TileMap = null) -> void:
	var world_bounds := compute_world_bounds(level_node, generated_root, generated_bounds, tile_layer, tile_map)
...
static func compute_world_bounds(level_node: Node2D, generated_root: Node2D, generated_bounds: Rect2, tile_layer: TileMapLayer = null, tile_map: TileMap = null) -> Rect2:
	var min_x := INF
	var min_y := INF
	var max_x := -INF
	var max_y := -INF
	...
	var local_tile_layer := tile_layer
	if local_tile_layer == null:
		local_tile_layer = level_node.get_node_or_null(^"Layer0") as TileMapLayer
	if local_tile_layer != null:
		var used_layer := local_tile_layer.get_used_rect()
		if used_layer.size != Vector2i.ZERO and local_tile_layer.tile_set != null:
		...
		var local_tile_map := tile_map
		if local_tile_map == null:
			local_tile_map = level_node.get_node_or_null(^"TileMap") as TileMap
		if local_tile_map != null:
			var used := local_tile_map.get_used_rect()
			if used.size != Vector2i.ZERO and local_tile_map.tile_set != null:
